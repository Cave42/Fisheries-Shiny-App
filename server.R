...
quad.plot.WC <- data.frame(read.csv(paste0(
  getwd(),
  "/Fisheries_Updated_File.csv"
)))

quad.plot.WCAnimation <- data.frame(read.csv(paste0(
  getwd(),
  "/Animation_Excel_File.csv"
)))

ExampleFile1 <- data.frame(read.csv(paste0(getwd(), "/ExampleFile1.csv")))

ExampleFileAnimation <- data.frame(read.csv(paste0(
  getwd(),
  "/Animation_Excel_File.csv"
)))
...
function(input, output, session) {
    ...
    if (input$customFile1 == TRUE && (is.null(input$fileInput1) == FALSE)) {
      ...
      a <-
        subset(
          quad.plot.WC,
          Abb2 %in%
            c(input$Flatfishes2, input$Scorpaenids2, input$Other2) &
      ...
      input$customFile1 == FALSE || (is.null(input$fileInput1) == TRUE)
    ) {
  ...
  output$distPlot <- renderPlot({
    ...
    quad.plot.WC4 <- filter(
      quad.plot.WC3 %>% group_by(Abb2) %>% top_n(1, Assessment_Year)
    )

    #Bottom only
    quad.plot.WC5 <- filter(
      quad.plot.WC3 %>% group_by(Abb2) %>% top_n(-1, Assessment_Year)
    )
    ...
    g <- g +
      ...
      geom_text(
        aes(
          x = as.numeric(vlineVariables[[1]]) + .01,
          label = vlineNames[[1]],
          y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2
        ),
        colour = vlineColors[[1]],
        angle = 90,
        text = element_text(size = 12)
      ) +
      geom_text(
        aes(
          x = as.numeric(vlineVariables[[2]]) + .01,
          label = vlineNames[[2]],
          y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2
        ),
        colour = vlineColors[[2]],
        angle = 90,
        text = element_text(size = 12)
      ) +
      if (vlineVariables[[3]] != " ") {
        geom_text(
          aes(
            x = as.numeric(vlineVariables[[3]]) + .01,
            label = vlineNames[[3]],
            y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2
          ),
          colour = vlineColors[[3]],
          angle = 90,
          text = element_text(size = 12)
        )
      }
    ...
    if (vlineVariables[[4]] != " ") {
      g <- g +
        geom_text(
          aes(
            x = as.numeric(vlineVariables[[4]]) + .01,
            label = vlineNames[[4]],
            y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2
          ),
          colour = vlineColors[[4]],
          angle = 90,
          text = element_text(size = 12)
        )
    }
    ...
    if (vlineVariables[[5]] != " ") {
      g <- g +
        geom_text(
          aes(
            x = as.numeric(vlineVariables[[5]]) + .01,
            label = vlineNames[[5]],
            y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2
          ),
          colour = vlineColors[[5]],
          angle = 90,
          text = element_text(size = 12)
        )
    }
    ...
    if (input$Id073 == "F/Fmsy") {
      g <- g +
        aes(B.Bmsy, F.Fmsy) +
        labs(
          x = expression(bold("Relative Stock Status")),
          y = expression(bold("Fishing Intensity (F/Fmsy)"))
        )
      ...
      g <- g +
        ...
        labs(
          x = expression(bold("Relative Stock Status")),
          y = expression(bold("Fishing Intensity (TM_ABC)"))
        )
    }
    ...
    if (all(c("Images") %in% input$ImageOptions)) {
      if (input$imageSize == "S") {
        g <- g +
          geom_image(aes(image = Image), size = .01, asp = 30 / 9) +
          scale_size_identity()
      }

      if (input$imageSize == "M") {
        g <- g +
          geom_image(aes(image = Image), size = .02, asp = 30 / 9) +
          scale_size_identity()
      }

      if (input$imageSize == "L") {
        g <- g +
          geom_image(aes(image = Image), size = .03, asp = 30 / 9) +
          scale_size_identity()
      }
    } else {
    ...
    if (
      all(c("Year Labels") %in% input$ImageOptions) &&
        all(c("Species labels") %in% input$ImageOptions)
    ) {
      g <-
        g +
        aes(label = Abb2) +
        geom_text_repel(
          show.legend = FALSE,
          aes(label = paste(Abb2, ";", Assessment_Year), color = Spp_type)
        )
      ...
      g <-
        ...
        geom_text_repel(
          show.legend = FALSE,
          aes(label = Assessment_Year, color = Spp_type)
        )
    }
  ...
  output$distPlot2 <- renderPlotly({
    ...
    if (input$customFile2 == TRUE && (is.null(input$fileInput2) == FALSE)) {
      ...
      quadAnimation <- subset(
        quad.plot.WC,
        Abb2 %in% c(input$Flatfishes4, input$Scorpaenids4, input$Other4)
      )

      #}
    } else if (
      input$customFile2 == FALSE || (is.null(input$fileInput2) == TRUE)
    ) {
      #else{
      quadAnimation <- subset(
    ...
    p <- p %>%
      layout(
        xaxis = list(
          range = c(as.numeric(xValues[[1]]), as.numeric(xValues[[2]]))
        ),
        yaxis = list(
          range = c(as.numeric(yValues[[1]]), as.numeric(yValues[[2]]))
        )
      )
    ...
    p <- p %>%
      add_annotations(
        x = as.numeric(vlineVariables[[1]]) + movement,
        y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2,
        text = vlineNames[[1]],
        font = list(color = vlineColors[[1]]),
        showarrow = F,
        textangle = 270
      )
    ...
    p <- p %>%
      add_annotations(
        x = as.numeric(vlineVariables[[2]]) + movement,
        y = (as.numeric(yValues[[2]]) - as.numeric(yValues[[1]])) / 2,
        text = vlineNames[[2]],
        font = list(color = vlineColors[[2]]),
        showarrow = F,
        textangle = 270
      )
    ...
    if (vlineVariables[[3]] != " ") {
      ...
      vline3 <- function(y = 0, color = "black") {
        list(
          type = "line",
          x0 = 0,
          x1 = 1,
    ...
    if (vlineVariables[[4]] != " ") {
      ...
      vline4 <- function(y = 0, color = "black") {
        list(
          type = "line",
          x0 = 0,
          x1 = 1,
